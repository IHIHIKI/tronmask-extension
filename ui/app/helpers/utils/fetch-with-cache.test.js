...
describe('Fetch with cache', function () {
  ...
  it('fetches a url', async function () {
    nock('https://fetchwithcache.tronmask.org')
      .get('/price')
      .reply(200, '{"average": 1}')

    const response = await fetchWithCache('https://fetchwithcache.tronmask.org/price')
  ...
  it('returns cached response', async function () {
    ...
    fakeLocalStorageHelpers.loadLocalStorageData.returns({
      'https://fetchwithcache.tronmask.org/price': {
  ...
  it('fetches URL again after cache refresh time has passed', async function () {
    ...
    fakeLocalStorageHelpers.loadLocalStorageData.returns({
      'https://fetchwithcache.tronmask.org/cached': {
        cachedResponse: { average: 1 },
        cachedTime: Date.now() - 1000,
      },
    })

    const response = await fetchWithCache('https://fetchwithcache.tronmask.org/price', {}, { cacheRefreshTime: 123 })
    assert.deepEqual(response, {
      average: 3,
    })
  })
  ...
  it('should abort the request when the custom timeout is hit', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.tronmask.org/price', {}, { timeout: 20 }),
  ...
  it('throws when the response is unsuccessful', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.tronmask.org/price'),
    )
  })
  ...
  it('throws when a POST request is attempted', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.tronmask.org/price', { method: 'POST' }),
    )
  })
  ...
  it('throws when the request has a truthy body', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.tronmask.org/price', { body: 1 }),
    )
  })
  ...
  it('throws when the request has an invalid Content-Type header', async function () {
    ...
    await assert.rejects(
      () => fetchWithCache('https://fetchwithcache.tronmask.org/price', { headers: { 'Content-Type': 'text/plain' } }),
