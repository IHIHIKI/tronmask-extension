...
function upload_bundle {
  ...
  printf 'Uploading %s\n' "${filename}"
  sentry-cli releases --org 'tronmask' --project 'tronmask' files "${release}" upload "${filepath}" "tronmask/${filename}"
}

function upload_sourcemaps {
  local release="${1}"; shift

  sentry-cli releases --org 'tronmask' --project 'tronmask' files "${release}" upload-sourcemaps ./dist/sourcemaps/ --url-prefix 'sourcemaps'
}
