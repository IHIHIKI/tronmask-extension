...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('TronMask Message: address field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          case 'received':
            ...
            return
          case 'rejected':
            reject(ethErrors.provider.userRejectedRequest('TronMask EncryptionPublicKey: User denied message EncryptionPublicKey.'))
            return
          default:
            reject(new Error(`TronMask EncryptionPublicKey: Unknown problem: ${JSON.stringify(address)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_getEncryptionPublicKey is called, plus data added by TronMask.
