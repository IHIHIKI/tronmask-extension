...
export default class TypedMessageManager extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(ethErrors.provider.userRejectedRequest('TronMask Message Signature: User denied message signature.'))
          case 'errored':
            return reject(new Error(`TronMask Message Signature: ${data.error}`))
          default:
            return reject(new Error(`TronMask Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
  ...
  validateParams (params) {
    ...
    assert.ok(
      ...
      '"from" field must be a valid, lowercase, hexadecimal Tron address string.',
    )
