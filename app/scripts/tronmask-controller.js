...
import KeyringController from '@opentron/keyring-controller'
...
export default class TronmaskController extends EventEmitter {
        ...
        filteredAccountTokens[checksummedAddress][networkType] = networkType === 'mainnet'
          ? (
            accountTokens[address][networkType].filter(({ address: tokenAddress }) => {
              ...
              return contractMap[checksumAddress] ? contractMap[checksumAddress].trc20 : true
            })
          )
  ...
  */
  ...
  async updateAndSetCustomRpc (rpcUrl, chainId, ticker = 'TRX', nickname, rpcPrefs) {
   ...
   * @returns {Promise<String>} - The RPC Target URL confirmed.
   */
  async setCustomRpc (rpcTarget, chainId, ticker = 'TRX', nickname = '', rpcPrefs = {}) {
